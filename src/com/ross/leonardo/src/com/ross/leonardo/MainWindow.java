...
public class MainWindow extends JFrame {
    ...
    public void maybeShowDonationPrompt() {
        boolean alreadyPrompted = Boolean.parseBoolean(
                config.getProperty("donationPromptShown", "false")
        );

        boolean alreadyDonated = Boolean.parseBoolean(
                config.getProperty("donated", "false")
        );

        if (alreadyPrompted || alreadyDonated) {
            return;
        }

        config.setProperty("donationPromptShown", "true");
        saveConfig();

        Object[] options = {"Donate", "Maybe Later"};

        int choice = JOptionPane.showOptionDialog(
                this,
                "<html><div style='width:320px;'>" +
                        "<b>If Leonardo saved you time, please consider supporting its development.</b><br><br>" +
                        "Leonardo is free and open source. Donations help support future improvements." +
                        "</div></html>",
                "Support Leonardo",
        ...
        if (choice == 0) {
            config.setProperty("donated", "true");
            saveConfig();
            openWebsite(AppInfo.DONATE_URL);
        }
    }
    ...
    private void setAppIcon() {
        try {
            Image img = AppInfo.getAppImage();
            if (img == null) {
                System.out.println("Leonardo icon not found on classpath.");
                return;
            }
